...
train_Y = [x["answer_type"] for x in train_qns_en]
test_X = [x["question"] for x in test_qns_en]
test_Y = [x["answer_type"] for x in test_qns_en]
...
train_Y=[ar_y(answer_types[x]) for x in train_Y]
test_Y=[ar_y(answer_types[x]) for x in test_Y]
...
for text_batch, label_batch in train_dataset.take(1):
  for i in range(3):
    ...
    for j in range(2):
      if label[j]==1:
        print(f'Label : {label} ({rev_answer_types[j]})')
  ...
  for result in results:
    ...
    for i in range(2):
      if result[i]>result[index]:
        index=i
    j=j+1
    print(input,rev_answer_types[index])
